...
topFEdict = {'2015': {'Car Model': name, 'Average Fuel Economy': max}}
print(topFEdict)
...
plt.title("Relationship between Combined FE and Combined CO2 (2015)")
...
mzdf.groupby(['FullCarline']).sum().plot(kind='pie', y='Combined CO2', autopct="%1.0f%%", title = 'Total Combined CO2 for each Aston Martin Type')
plt.show()
"""
...
mmcdf.groupby(['FullCarline']).sum().plot(kind='pie', y='Combined CO2', autopct="%1.0f%%", title = 'Total Combined CO2 for each Aston Martin Type')
plt.show()
"""
...
plt.scatter(x, y, label='Data')
plt.title("Relationship between Engine Power and FE rating")
